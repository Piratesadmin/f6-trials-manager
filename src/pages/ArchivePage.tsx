...
export function ArchivePage({ seasonSettings, players, sessions, archives, archivedPlayers, rollover, cleanupTrialists, restoreArchivedPlayer }: Props) {
  ...
  const [archiveTeam,setArchiveTeam]=useState('All teams')
  ...
  const archiveTeams=useMemo(()=>[...new Set(archivedPlayers.map(record=>record.player.appliedTeam).filter(Boolean))].sort(),[archivedPlayers])
  ...
  const filteredArchived=useMemo(()=>archivedPlayers.filter(record=>{
    ...
    const search=`${player.name} ${player.email} ${player.position} ${player.secondaryPosition} ${player.appliedTeam} ${player.recommendation} ${player.strengths} ${player.playingExperience}`.toLowerCase()
    return (!archiveQuery.trim()||search.includes(archiveQuery.trim().toLowerCase()))&&(archiveTeam==='All teams'||player.appliedTeam===archiveTeam)&&(archivePosition==='All positions'||player.position===archivePosition||player.secondaryPosition===archivePosition)
  }),[archivedPlayers,archiveQuery,archiveTeam,archivePosition])
