...
import { CalendarDays, Check, CheckCircle2, Clock3, Mail, MapPin, Phone, Search, ShieldCheck, UserPlus, Volleyball, X } from 'lucide-react'
...
type Props={signups:ClubSignup[];ready:boolean;loadError:string;readOnly:boolean;updateStatus:(id:string,status:ClubSignupStatus,outcome:ClubSignupOutcome|'')=>Promise<void>}
type PendingStatus={signupId:string;playerName:string;status:Exclude<ClubSignupStatus,'new'>;outcome:ClubSignupOutcome|''}
...
export function SignupInboxPage({signups,ready,loadError,readOnly,updateStatus}:Props){
  ...
  const saveStatus=async(id:string,next:ClubSignupStatus,outcome:ClubSignupOutcome|'')=>{setBusy(true);setError('');try{await updateStatus(id,next,outcome);return true}catch{setError('The sign-up status could not be updated. Please try again.');return false}finally{setBusy(false)}}
  const requestStatus=async(next:ClubSignupStatus)=>{if(!selected||readOnly||next===selected.status)return;if(next==='new'){await saveStatus(selected.id,next,'');return}setError('');setPendingStatus({signupId:selected.id,playerName:selected.name,status:next,outcome:''})}
  const confirmStatus=async()=>{if(!pendingStatus||!pendingStatus.outcome)return;if(await saveStatus(pendingStatus.signupId,pendingStatus.status,pendingStatus.outcome))setPendingStatus(null)}
  ...
  return <>
    ...
    <section className="signup-manager-layout panel">
      ...
      <div className="signup-manager-detail">{selected?<><header><div className="signup-detail-identity">{selected.profilePhoto?<img src={selected.profilePhoto} alt={`${selected.name} profile`}/>:<span>{selected.name.split(' ').map(part=>part[0]).join('').slice(0,2)}</span>}<div><span className={`signup-status ${selected.status}`}>{clubSignupStatusLabel(selected.status)}</span><h2>{selected.name}</h2><p>Submitted {date(selected.createdAt)} · {selected.id}</p></div></div>{!readOnly&&<select value={selected.status} disabled={busy} onChange={event=>void requestStatus(event.target.value as ClubSignupStatus)}>{clubSignupStatuses.map(value=><option value={value} key={value}>{clubSignupStatusLabel(value)}</option>)}</select>}</header>{error&&!pendingStatus&&<div className="signup-error">{error}</div>}
        ...
        {selected.statusOutcome&&<TextBlock title="Latest status outcome" value={clubSignupOutcomeLabel(selected.statusOutcome)}/>}
