...
function ConfirmedPlayerCard({player,editable,isAdmin,finance,financeSettings,onOpen,onPosition}:{player:Player;editable:boolean;isAdmin:boolean;finance:PlayerFinanceMap[string]|undefined;financeSettings:FinanceSettings;onOpen:()=>void;onPosition:(position:string)=>void}){
  ...
  return <div className={`confirmed-player-card ${editable?'position-editable':''}`}><button className="confirmed-player-summary" onClick={onOpen}><div className="confirmed-player-icon"><Check/></div><div><b>{player.name}</b><span>{confirmedPosition(player)}{player.bibNumber?` · #${player.bibNumber}`:''}</span></div><ArrowRight/></button>{editable&&<label className="confirmed-position-control"><span>Squad position</span><select aria-label={`${player.name} squad position`} value={confirmedPosition(player)} onChange={event=>onPosition(event.target.value)}>{['Unassigned',...positions].map(position=><option key={position}>{position}</option>)}</select></label>}{isAdmin?<div className="confirmed-finance"><span className={`finance-status ${deadline.state==='overdue'?'overdue':status.toLowerCase().replaceAll(' ','-')}`}>{deadline.state==='overdue'?'Payment overdue':status}</span><small>{deadline.state==='overdue'?deadline.label:`${formatCurrency(outstandingAmount(record,amountOwed))} outstanding`}</small></div>:<span className="confirmed-private">Confirmed</span>}</div>
}
