...
function CoachTimesheets({currentUid,currentEmail,currentProfile,currentSeason,hourlyRates,entries,invoices,saveEntry,deleteEntry,submitInvoice}:Props){
  ...
  const [form,setForm]=useState<{date:string;team:string;activity:TimesheetActivity;hours:string;minutes:string;notes:string}>(()=>({date:today(),team:assignedTeams[0]||'',activity:'Training',hours:'1',minutes:'30',notes:''}))
  ...
  const otherTeams=teams.filter(team=>!assignedTeams.includes(team))
  const helpingAnotherTeam=Boolean(form.team&&!assignedTeams.includes(form.team))
  const usualRate=hourlyRateForTeam(rate,assignedTeams[0]||'')
  ...
  const add=async(event:FormEvent)=>{
    ...
    if(!currentProfile||!form.team||!form.date)return
    if(!Number.isInteger(hours)||!Number.isInteger(minutes)||hours<0||hours>24||minutes<0||minutes>59||totalMinutes<=0||totalMinutes>24*60){setError('Enter a duration between 1 minute and 24 hours.');return}
    setBusy('add');setNotice('');setError('')
    try{
      const timestamp=Date.now()
      await saveEntry({id:crypto.randomUUID(),coachUid:currentUid,coachName:currentProfile.displayName,coachEmail:currentEmail,date:form.date,team:form.team,activity:form.activity,hours:totalMinutes/60,notes:form.notes.trim(),season:currentSeason,invoiceId:'',createdAt:timestamp,updatedAt:timestamp})
      setForm(current=>({...current,date:today(),hours:'1',minutes:'30',notes:''}));setNotice('Time added to your draft timesheet.')
  ...
  return <>
    <PageHeader title="Timesheets" subtitle={`Record coaching time for any club team and submit it to the treasurer for ${currentSeason}.`}/>
    ...
    <div className="timesheet-coach-layout">
      <section className="panel timesheet-entry-panel"><header><div><span className="eyebrow">NEW ENTRY</span><h2>Add coaching time</h2></div><Plus/></header><form onSubmit={add}><label>Date<input type="date" max={today()} required value={form.date} onChange={event=>setForm({...form,date:event.target.value})}/></label><label>Team<select required value={form.team} onChange={event=>setForm({...form,team:event.target.value})}><option value="">Choose team</option>{assignedTeams.length>0&&<optgroup label="Your teams">{assignedTeams.map(team=><option key={team}>{team}</option>)}</optgroup>}{otherTeams.length>0&&<optgroup label="Helping another team">{otherTeams.map(team=><option key={team}>{team}</option>)}</optgroup>}</select>{helpingAnotherTeam&&<small className="timesheet-helping-note">Helping {form.team} · paid at your usual {usualRate?`${formatCurrency(usualRate)}/hr`:'rate once set by the treasurer'}.</small>}</label><label>Activity<select value={form.activity} onChange={event=>setForm({...form,activity:event.target.value as TimesheetActivity})}>{timesheetActivities.map(activity=><option key={activity}>{activity}</option>)}</select></label><fieldset className="timesheet-duration"><legend>Duration</legend><div><label>Hours<input aria-label="Duration hours" type="number" min="0" max="24" step="1" required value={form.hours} onChange={event=>setForm({...form,hours:event.target.value})}/></label><label>Minutes<input aria-label="Duration minutes" type="number" min="0" max="59" step="1" required value={form.minutes} onChange={event=>setForm({...form,minutes:event.target.value})}/></label></div></fieldset><label className="timesheet-notes">Work completed<textarea maxLength={500} rows={4} value={form.notes} onChange={event=>setForm({...form,notes:event.target.value})} placeholder="Session, fixture, age group, or other useful detail…"/></label><button className="primary" disabled={busy==='add'||!assignedTeams.length}>{busy==='add'?'Adding…':<><Plus/>Add to timesheet</>}</button>{!assignedTeams.length&&<p className="timesheet-form-help">Ask an administrator to assign this account to a team and set your usual rate.</p>}</form></section>
