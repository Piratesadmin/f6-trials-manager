...
export function OfferOptionsEditor({ player, save, compact = false, disabled = false }: Props) {
  ...
  return <section className={`offer-options-editor ${compact?'compact':''}`}>
    ...
    {offers.length?<div className="offer-option-list">{offers.map((offer,index)=><article key={offer.team}><div className="offer-option-number">{index+1}</div><div className="offer-option-title"><b>{offer.team}</b><small>{offer.team===player.appliedTeam?'Original applied team':'Alternative team option'}</small></div><label>Position<select disabled={disabled} value={offer.position} onChange={event=>updateOffer(offer.team,'position',event.target.value)}>{positions.map(position=><option key={position}>{position}</option>)}</select></label><label>Squad role<select disabled={disabled} value={offer.squadRole} onChange={event=>updateOffer(offer.team,'squadRole',event.target.value)}>{squadRoles.map(role=><option key={role}>{role}</option>)}</select></label><button className="remove-offer-option" disabled={disabled} type="button" onClick={()=>toggleTeam(offer.team)} aria-label={`Remove ${offer.team} offer`}><Trash2/></button></article>)}</div>:<div className="offer-options-empty"><Layers3/><b>No team selected</b><span>Choose at least one team before reviewing the offer email.</span></div>}
  </section>
}
