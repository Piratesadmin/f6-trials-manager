...
export function Sidebar({page,setPage,players,teamFilter,openTeam,syncState,signedIn,accountEmail,accountName,sharedAccount,assignedTeams,isAdmin,accountRole,currentSeason,trialsMode,onSignOut}:Props){
  return <aside className="sidebar">
    ...
    <div className="team-list"><p>TEAMS</p>{teams.map(team=><button key={team} className={teamFilter===team?'team-active':''} onClick={()=>openTeam(team)}>{team}<span>{players.filter(player=>(trialsMode?player.appliedTeam:confirmedTeam(player))===team).length}</span></button>)}</div>
