...
import { AlertTriangle, CalendarDays, CheckCircle2, FileSpreadsheet, Plus, RefreshCw, Upload, Users, X } from 'lucide-react'
...
import { formatSessionDate, trialSessionMatchesImport } from '../utils/schedule'
...
type Props = {
  ...
  onWorkbookImport: (session: Omit<TrialSession, 'id'>, players: Omit<Player, 'id'>[], existingSessionId?: string) => Promise<void>
}
...
export function CsvImportModal({ existingPlayers, existingSessions, onClose, onImport, onWorkbookImport }: Props) {
  ...
  const [eventImportMode,setEventImportMode]=useState<'create'|'update'>('create')
  ...
  const detectedMatchingSession = workbookSession ? existingSessions.find(session=>trialSessionMatchesImport(session,workbookSession)) : undefined
  const matchingSession = existingSessions.find(session=>session.id===matchedSessionId)||detectedMatchingSession

  const reset = () => {
    setParsed(null); setWorkbook(null); setWorkbookSession(null); setFileName(''); setMapping(blankMapping); setError(''); setEventImportMode('create'); setMatchedSessionId('')
  ...
  const readFile = async (file?: File) => {
    ...
    try {
      if (file.name.toLowerCase().endsWith('.xlsx')) {
        ...
        const matching=result.session?existingSessions.find(session=>trialSessionMatchesImport(session,result.session!)):undefined
        setMatchedSessionId(matching?.id||'')
        setEventImportMode(matching?'update':'create')
        setParsed(null)
      } else {
  ...
  const submitWorkbook = async () => {
    ...
    setBusy(true); setError('')
    try {
      if (workbook.mode === 'trial-session' && workbookSession) await onWorkbookImport(workbookSession, workbookReady, eventImportMode==='update'?matchingSession?.id:undefined)
    ...
    <section className="import-modal" role="dialog" aria-modal="true" aria-labelledby="import-title">
      ...
      {!parsed && !workbook ? <div className="upload-step">
        ...
        {workbook ? <>
          ...
          {workbook.mode==='trial-session'&&matchingSession&&<div className="existing-event-import"><div><RefreshCw/><span><b>Matching event found</b><small>{formatSessionDate(matchingSession.date)} · {matchingSession.startTime||'Time not set'} · {matchingSession.title}</small></span></div><div className="existing-event-options"><button type="button" className={eventImportMode==='update'?'selected':''} onClick={()=>setEventImportMode('update')}><RefreshCw/><span><b>Update existing event</b><small>Refresh its details and add or update the players in this workbook. Existing players not listed are retained.</small></span></button><button type="button" className={eventImportMode==='create'?'selected':''} onClick={()=>setEventImportMode('create')}><Plus/><span><b>Create a separate event</b><small>Keep the matching event unchanged and create another event with this roster.</small></span></button></div></div>}
          ...
          {error && <div className="import-alert error"><AlertTriangle/>{error}</div>}
          <div className="modal-actions"><button className="secondary" onClick={onClose}>Cancel</button><button className="primary" disabled={busy||!workbookReady.length} onClick={submitWorkbook}><Upload/>{busy?'Importing…':workbook.mode==='trial-session'&&matchingSession&&eventImportMode==='update'?`Update event and import ${workbookReady.length} players`:workbook.mode==='trial-session'?`Create session and import ${workbookReady.length} players`:`Import ${workbookReady.length} players`}</button></div>
        </> : parsed ? <>
