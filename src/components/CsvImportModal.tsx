...
const blankMapping: CsvMapping = { name: '', firstName: '', lastName: '', email: '', appliedTeam: '', position: '', trialDate: '' }
...
export function CsvImportModal({ existingPlayers, onClose, onImport }: Props) {
  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={event => event.target === event.currentTarget && onClose()}>
    <section className="import-modal" role="dialog" aria-modal="true" aria-labelledby="csv-title">
      <div className="modal-head"><div><span className="eyebrow">CSV IMPORT</span><h2 id="csv-title">Import trial sign-ups</h2><p>Upload a CSV exported from Google Forms or Google Sheets.</p></div><button className="modal-close" onClick={onClose} aria-label="Close"><X /></button></div>
      ...
      {!parsed ? <div className="upload-step">
        ...
        <div className="mapping-grid">
          <div><span className="eyebrow">MATCH COLUMNS</span><h3>Tell us what each column contains</h3><p>We have automatically matched likely headings. Check them before importing.</p></div>
        ...
        <div className="preview-table-wrap"><table className="preview-table"><thead><tr><th>Status</th><th>Name</th><th>Email</th><th>Team</th><th>Position</th><th>Trial</th></tr></thead><tbody>{analysed.slice(0,8).map(({player,duplicate,valid}, index) => <tr key={`${player.email}-${index}`}><td><span className={`row-status ${!valid?'invalid':duplicate?'duplicate':'ready'}`}>{!valid?'Invalid':duplicate?'Duplicate':'Ready'}</span></td><td>{player.name || 'Missing name'}</td><td>{player.email || 'Missing email'}</td><td>{player.appliedTeam}</td><td>{player.position}</td><td>{player.trialDate}</td></tr>)}</tbody></table>{analysed.length > 8 && <p className="preview-more">Showing 8 of {analysed.length} rows</p>}</div>
        {error && <div className="import-alert error"><AlertTriangle/>{error}</div>}
