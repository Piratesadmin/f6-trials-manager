...
import { averageRatingOutOfTen, isAssessed } from '../utils/player'

export function StatsCards({ players }: { players: Player[] }) {
  const assessed = players.filter(isAssessed)
  const clubAverage = assessed.length ? assessed.reduce((total, player) => total + averageRatingOutOfTen(player), 0) / assessed.length : 0
  ...
  const stats = [
    ...
    { label: 'Average rating', value: clubAverage ? `${clubAverage.toFixed(1)}/10` : '—', detail: 'Five-star records converted proportionally', icon: Star },
