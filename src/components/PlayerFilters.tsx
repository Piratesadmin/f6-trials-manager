...
export function PlayerFilters({ filters, setFilters, sessions, onClose }: Props) {
  ...
  return <section className="player-filter-panel" aria-label="Player filters">
    ...
    <div className="filter-grid">
      <label>Trial session<select value={filters.sessionId} onChange={event => setFilters({ ...filters, sessionId: event.target.value })}><option value="all">Any trial session</option><option value="unassigned">No trial events</option>{[...sessions].filter(session=>session.eventType==='trial').sort((a,b)=>a.date.localeCompare(b.date)).map(session => <option key={session.id} value={session.id}>{formatSessionDate(session.date)} · {session.title}</option>)}</select></label>
    ...
    <div className="rating-filter"><div><b>Minimum average rating</b><span>{filters.minimumRating ? `${filters.minimumRating}+/10` : 'Any rating'}</span></div><input aria-label="Minimum average rating out of 10" type="range" min="0" max="10" step="1" value={filters.minimumRating} onChange={event => setFilters({ ...filters, minimumRating: Number(event.target.value) })}/><div className="rating-scale"><span>Any</span><span>1</span><span>2</span><span>3</span><span>4</span><span>5</span><span>6</span><span>7</span><span>8</span><span>9</span><span>10</span></div></div>
