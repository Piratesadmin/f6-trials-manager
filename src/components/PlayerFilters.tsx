...
export function PlayerFilters({ filters, setFilters, sessions, onClose }: Props) {
  ...
  return <section className="player-filter-panel" aria-label="Player filters">
    ...
    <div className="filter-grid">
      <label>Trial session<select value={filters.sessionId} onChange={event => setFilters({ ...filters, sessionId: event.target.value })}><option value="all">Any trial session</option><option value="unassigned">Not assigned</option>{[...sessions].sort((a,b)=>a.date.localeCompare(b.date)).map(session => <option key={session.id} value={session.id}>{formatSessionDate(session.date)} · {session.title}</option>)}</select></label>
